...
async function demonstrateEquipmentSystem(): Promise<void> {
  ...
  const ring1 = await generateItemFromBase(ACCESSORY_BASE_TYPES.find(a => a.name === "Iron Ring")!, 20);
  const ring2 = await generateItemFromBase(ACCESSORY_BASE_TYPES.find(a => a.name === "Gold Ring")!, 22);
